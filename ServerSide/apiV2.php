...
try
{
    ...
    if($command == "play")
    {
        ...
        if(isset($_GET["ball1_position_x"]) && isset($_GET["ball1_position_y"])&& isset($_GET["ball1_speed_x"]) && isset($_GET["ball1_speed_y"]))
        {
            $ball_id = 1;
            $ball1_position_x = $_GET["ball1_position_x"];
            $ball1_position_y = $_GET["ball1_position_y"];
            $api->updateBallPosition($ball_id,$ball1_position_x,$ball1_position_y);
            $ball1_speed_x = $_GET["ball1_speed_x"];
            $ball1_speed_y = $_GET["ball1_speed_y"];
            $api->updateBallSpeed($ball_id,$ball1_speed_x,$ball1_speed_y);
            $ball_indicate  = $ball_indicate + 1;
        }
        
        // IF WRITE THE BALL NUMBER TWO
        if(isset($_GET["ball2_position_x"]) && isset($_GET["ball2_position_y"])&& isset($_GET["ball2_speed_x"]) && isset($_GET["ball2_speed_y"]))
        {
            $ball_id = 2;
            $ball2_position_x = $_GET["ball2_position_x"];
            $ball2_position_y = $_GET["ball2_position_y"];
            $api->updateBallPosition($ball_id,$ball2_position_x,$ball2_position_y);
            $ball2_speed_x = $_GET["ball2_speed_x"];
            $ball2_speed_y = $_GET["ball2_speed_y"];
            $api->updateBallSpeed($ball_id,$ball2_speed_x,$ball2_speed_y);
            $ball_indicate = $ball_indicate * -1;
        }
        
        // IF USER WANTS TO CHANGE OWNERSHIP
        if(isset($_GET["ball_id"]) && isset($_GET["player_name"]))
        {
            $ball_id = $_GET["ball_id"];
            $new_owner_name = $_GET["player_name"];
...
class API
{
    ...
    function getRivalBall($ball_indicate)
    {
        ...
        else if($ball_indicate == 1)
        {
            ...
            return array('ball1' => $pos1, 'ball2' => $pos2);
        }
